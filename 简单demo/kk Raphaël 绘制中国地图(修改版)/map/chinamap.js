...
//                tiplayer.text(china[state]['name']+":0").css({ 'opacity': '0.75', 'top': (e.pageY + 10) + 'px', 'left': (e.pageX + 10) + 'px' }).fadeIn('normal');
                hovertip(china[state]['name'],e);
          //      $sl.css("font-size", "20px");
...
//                    tiplayer.text(china[state]['name']+":0").css({ 'opacity': '0.75', 'top': (e.pageY + 10) + 'px', 'left': (e.pageX + 10) + 'px' }).fadeIn('normal');
           //         $sl.css("font-size", "20px");
            ...
            function clickMap() {
            	//console.log(china[state]['path']);
            //	console.log(china[state]);
//            	alert(china[state]['name']);
            //	console.log(china[state]['path'].id);
                if (current == state)
                    return;
                //重置上次点击的图形
    ...
    function hovertip(provicename,e){
    	
    	var numvalue='0';
    	
    	$.each(datagetfrom, function (i, item) {
    		if(provicename==item.AreaName){
    			numvalue=item.sum;
    		}
        });
    	
    	
    	tiplayer.text(provicename+":"+numvalue).css({ 'opacity': '0.75', 'top': (e.pageY + 10) + 'px', 'left': (e.pageX + 10) + 'px' }).fadeIn('normal');	
    }

}
var datagetfrom=null;
...
//        url: "./mapList.js",   //jsonp
        ...
        success: function (data) {
        	datagetfrom=data;
//        	console.log(datagetfrom);
            var html = "";
            $.each(data, function (i, item) {
//            	console.log(china[item.AreaName]);
            //	console.log(data);
